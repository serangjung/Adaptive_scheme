...
% FM parameters
 ...
 field_numbers = 1:(qv_length-6)/2;
 repeated_numbers = repmat(field_numbers, 2, 1);
 field_result = repeated_numbers(:)';

 inputFM.field_index = horzcat(zeros(1,6),field_result);
 
% Learn FFM
tic
inputFM=FFM_xlearn_cv_ml_JSR(inputFM);
caltime_fm=toc;
...
if strcmp(input.fm_optimizer,'simulated_annealing')
    ...
    SA_name = sprintf('%s_cal_fm_SA_time.mat',filename);
    ...
    qv_gmin_qa=QA_answer;    opt_file = fopen(sprintf('opt_%s.txt',fntd),'w');
