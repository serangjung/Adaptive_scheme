function output=ml_opt_per_batch_digit_FFM_JSR(input,ttn)
...
if isfield(input,'sampling_tag')==1
    if isfield(input,'csv_mode')==1
        if strcmp(input.csv_mode, 'on')
            ...
            %====================here making LIBFFM ================
            [os,oi]=system(sprintf('C:\\Users\\user\\miniconda3\\envs\\JSR_jupyter_desktop\\python -c "from LIBFFM_generator import LIBFFM_generator; LIBFFM_generator(%s)"',"'"+string(fntd)+"'"));
            [os,oi]=system(sprintf('C:\\Users\\user\\miniconda3\\envs\\JSR_jupyter_desktop\\python -c "from LIBFFM_generator import LIBFFM_generator; LIBFFM_generator(%s)"',"'"+string(fntd)+"_cv'"));
