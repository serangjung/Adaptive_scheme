...
elseif length(varargin)==1
    ...
    % xlearncsvsfile = fopen(sprintf('%s_cv.txt',name),'w');
    % clear get_size_vector;
    % get_size_vector=size(picked_qv_ii_array);
    % labelvalue=num2cell((picked_FOM));
    % 
    % labeltext='%12.8f ';
    % for gn=1:get_size_vector(2)
    %     labelvalue(gn+1,:)=num2cell(transpose(picked_qv_ii_array(:,gn)));
    %     labeltext=[labeltext '%d '];
    % end
    % labeltext=[labeltext '\n'];
    % fprintf(xlearncsvsfile,labeltext,labelvalue{:});
